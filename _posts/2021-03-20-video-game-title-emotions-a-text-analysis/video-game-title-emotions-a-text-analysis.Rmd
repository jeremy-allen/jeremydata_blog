...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap='Video Game Title Emotions', layout="l-screen-inset"}

knitr::include_graphics("title_emotions.png")

```

## Video Game Title Emotions - a text analysis

This week's [Tidy Tuesday challenge](https://github.com/rfordatascience/tidytuesday) offered video game data for analysis. I analyzed the text in the video game titles. Using the [NRC Emotion Lexicon](https://saifmohammad.com/WebPages/NRC-Emotion-Lexicon.htm) I measured the joy words and sad words in the titles by year. 

I used the [tidytext package](https://www.tidytextmining.com/) to tokenize each years-worth of game titles and remove stop words and plotted the polar plots with the [gpglot2](https://www.tidyverse.org/) package. Then I made word clouds using [wordcloud2](https://cran.r-project.org/web/packages/wordcloud2/vignettes/wordcloud.html). The [clowplot package](https://cran.r-project.org/web/packages/cowplot/vignettes/introduction.html) was used to place screenshots of the wordclouds within the final image. 

[Code is also on GitHub](https://github.com/jeremy-allen/tidy-tues)  


```{r echo=TRUE, eval=FALSE, include=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(wordcloud2)
library(cowplot)
library(tidyverse)
library(tidytext)
